...
upload_error($m){$de=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($de?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$de):""):'Plik nie istnieje.');}function
...
lang($nh,$ze=null){if(is_array($nh)){$qf=($ze==1?0:($ze%10>1&&$ze%10<5&&$ze/10%10!=1?1:2));$nh=$nh[$qf];}$nh=str_replace("%d","%s",$nh);$ze=format_number($ze);return
...
h($g->error);}function
check_sqlite_name($C){global$g;$yc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($yc)\$~",$C)){$g->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$yc));return
false;}return
true;}function
...
set_schema($hg){global$g,$vh,$Eg;$J=$g->query("SET search_path TO ".idf_escape($hg));foreach(types()as$U){if(!isset($vh[$U])){$vh[$U]=0;$Eg['Typy uÅ¼ytkownika'][]=$U;}}return$J;}function
...
unconvert_field($n,$J){return$J;}function
support($Ac){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Ac);}$w="oracle";$vh=array();$Eg=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$vh+=$X;$Eg[$x]=array_keys($X);}$Bh=array();$Le=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Qc=array("length","lower","round","upper");$Vc=array("avg","count","count distinct","max","min","sum");$ac=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$tf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Lg,$O=""){echo'<p class="links">';$Td=array("select"=>'PokaÅ¼ dane');if(support("table")||support("indexes"))$Td["table"]='Struktura tabeli';if(support("table")){if(is_view($Lg))$Td["view"]='ZmieÅ„ perspektywÄ™';else$Td["create"]='ZmieÅ„ tabelÄ™';}if($O!==null)$Td["edit"]='Nowy rekord';foreach($Td
...
selectQuery($H,$bh){global$w;return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($bh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".'Edytuj'."</a>":"")."</p>";}function
rowDescription($Q){return"";}function
rowDescriptions($L,$Ic){return$L;}function
selectLink($X,$n){}function
selectVal($X,$_,$n,$Xe){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$n["type"])&&!preg_match("~var~",$n["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$n["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($Xe));return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
editVal($X,$n){return$X;}function
selectColumnsPrint($M,$f){global$Qc,$Vc;print_fieldset("select",'pokaÅ¼',$M);$r=0;$M[""]=array();foreach($M
as$x=>$X){$X=$_GET["columns"][$x];$e=select_input(" name='columns[$r][col]' onchange='".($x!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$f,$X["col"]);echo"<div>".($Qc||$Vc?"<select name='columns[$r][fun]' onchange='helpClose();".($x!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array('Funkcje'=>$Qc,'Agregacje'=>$Vc)),$X["fun"])."</select>"."($e)":$e)."</div>\n";$r++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($Qe,$f,$v){print_fieldset("sort",'Sortuj',$Qe);$r=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$r]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'malejÄ…co')."</div>\n";$r++;}}echo"<div>".select_input(" name='order[$r]' onchange='selectAddRow(this);'",$f),checkbox("desc[$r]",1,false,'malejÄ…co')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($ah){if($ah!==null){echo"<fieldset><legend>".'DÅ‚ugoÅ›Ä‡ tekstu'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($ah)."'>","</div></fieldset>\n";}}function
...
messageQuery($H,$bh){global$w;restart_session();$ad=&get_session("queries");$s="sql-".count($ad[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$ad[$_GET["db"]][]=array($H,time(),$bh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'Zapytanie SQL'."</a>"."<div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.($bh?" <span class='time'>($bh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ad[$_GET["db"]])-1)).'">'.'Edytuj'.'</a>':'').'</div>';}function
...
';}$this->databasesPrint($oe);if(DB==""||!$oe){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'Zapytanie SQL'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'Import'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Eksport'."</a>\n";}if($_GET["ns"]!==""&&!$oe&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'UtwÃ³rz tabelÄ™'."</a>\n";if(!$S)echo"<p class='message'>".'Brak tabel.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($oe){global$b,$g;$j=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Eb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".'baza danych'."'>DB</span>: ".($j?"<select name='db'$Eb>".optionlist(array(""=>"")+$j,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Wybierz'."'".($j?" class='hidden'":"").">\n";if($oe!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".'Schemat'.": <select name='ns'$Eb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$Bg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q).">".'przeglÄ…daj'."</a> ";$C=$this->tableName($Bg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Bg)?"view":""))." title='".'Struktura tabeli'."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$vd=>$X){if($X[0]<$bh)unset($ud[$vd]);}}$td=&$ud[$b->bruteForceKey()];if(!$td)$td=array($bh+30*60,0);$td[1]++;$qg=serialize($ud);rewind($Nc);fwrite($Nc,$qg);ftruncate($Nc,strlen($qg));flock($Nc,LOCK_UN);fclose($Nc);}$Ia=$_POST["auth"];if($Ia){$ud=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$td=$ud[$b->bruteForceKey()];$xe=($td[1]>30?$td[0]-time():0);if($xe>0)auth_error(lang(array('Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minutÄ™.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minuty.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minut.'),ceil($xe/60)));session_regenerate_id();$l=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=(string)$Ia["password"];$k=$Ia["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ia["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$zf=$b->permanentLogin(true);$nf[$x]="$x:".base64_encode($zf?encrypt_string($G,$zf):"");cookie("adminer_permanent",implode(" ",$nf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($Zc&&!verify_token()){page_header('Wyloguj','NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Wylogowano pomyÅ›lnie.');}}elseif($nf&&!$_SESSION["pwds"]){session_regenerate_id();$zf=$b->permanentLogin();foreach($nf
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ra[$x]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($vh[$x]==254)$X="<code>$X</code>";}if(isset($Td[$x])&&!$f[$Td[$x]]){if($Te&&$w=="sql"){$Q=$K[array_search("table=",$Td)];$_=$Td[$x].urlencode($Te[$Q]!=""?$Te[$Q]:$Q);}else{$_="edit=".urlencode($Td[$x]);foreach($v[$Td[$x]]as$hb=>$Ad)$_.="&where".urlencode("[".bracket_escape($hb)."]")."=".urlencode($K[$Ad]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Brak rekordÃ³w.')."\n";return$J;}function
...
edit_type($x,$n,$jb,$Jc=array()){global$Eg,$vh,$Bh,$He;$U=$n["type"];echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($vh[$U])&&!isset($Jc[$U]))array_unshift($Eg,$U);if($Jc)$Eg['Klucze obce']=$Jc;echo
optionlist($Eg,$U),'</select>
<td><input name="',$x,'[length]" value="',h($n["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$n["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$x"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porÃ³wnywanie znakÃ³w'.')'.optionlist($jb,$n["collation"]).'</select>',($Bh?"<select name='$x"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Bh,$n["unsigned"]).'</select>':''),(isset($n['on_update'])?"<select name='$x"."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'W przypadku zmiany'.")","CURRENT_TIMESTAMP"),$n["on_update"]).'</select>':''),($Jc?"<select name='$x"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'W przypadku usuniÄ™cia'.")".optionlist(explode("|",$He),$n["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'Dodaj nastÄ™pny'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'Dodaj nastÄ™pny'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.1.0' alt='^' title='".'PrzesuÅ„ w gÃ³rÄ™'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.1.0' alt='v' title='".'PrzesuÅ„ w dÃ³Å‚'."'>&nbsp;":""),($Ue==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.1.0' alt='x' title='".'UsuÅ„'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'ZmieÅ„'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Og=array();$Pg=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Yd,PREG_SET_ORDER);foreach($Yd
...
as$C=>$n){if(!isset($n["privileges"][$Ch?"update":"insert"])||$b->fieldName($n)=="")unset($o[$C]);}if($_POST&&!$m&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Ch?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$yh=unique_array($_GET["where"],$v);$Hf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'Rekord zostaÅ‚ usuniÄ™ty.',$l->delete($a,$Hf,!$yh));else{$O=array();foreach($o
as$C=>$n){$X=process_input($n);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Ch){if(!$O)redirect($A);queries_redirect($A,'Rekord zostaÅ‚ zaktualizowany.',$l->update($a,$O,$Hf,!$yh));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$I=$l->insert($a,$O);$Ld=($I?last_id():0);queries_redirect($A,sprintf('Rekord%s zostaÅ‚ dodany.',($Ld?" $Ld":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($o
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($jc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".'skÅ‚adowanie'.")")+$jc,$K["Engine"])."</select>":""),' ',($jb&&!preg_match("~sqlite|mssql~",$w)?html_select("Collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$jb,$K["Collation"]):""),' <input type="submit" value="Zapisz zmiany">
';}echo'
...
',checkbox("defaults",1,true,'WartoÅ›ci domyÅ›lne',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($ob?" checked":"").">".'Komentarz'."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="UsuÅ„"',confirm(),'>';}if(support("partitioning")){$gf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",'Partycjonowanie',$K["partition_by"]);echo'<p>
...
Partycje: <input type="number" name="partitions" class="size',($gf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Gg=false;$Kd=$k;}}queries_redirect(ME."db=".urlencode($Kd),'Baza danych zostaÅ‚a utworzona.',$Gg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'Baza danych zostaÅ‚a zmieniona.');}}page_header(DB!=""?'ZmieÅ„ bazÄ™ danych':'UtwÃ³rz bazÄ™ danych',$m,array(),h(DB));$jb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$jb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Rc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Rc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($jb?html_select("collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$jb,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'Dodaj nastÄ™pny'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ge='Perspektywa zostaÅ‚a zmieniona.';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ge);else{$Wg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Wg).$Ea,"DROP VIEW ".table($Wg),($_POST["drop"]?substr(ME,0,-1):$A),'Perspektywa zostaÅ‚a usuniÄ™ta.',$ge,'Perspektywa zostaÅ‚a utworzona.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$g->error;}page_header(($a!=""?'ZmieÅ„ perspektywÄ™':'UtwÃ³rz perspektywÄ™'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>Wykonuj co<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$sd,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Cg,$K["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$bg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$m){$Wg="$K[name]_adminer_".uniqid();drop_create("DROP $bg ".idf_escape($da),create_routine($bg,$K),"DROP $bg ".idf_escape($K["name"]),create_routine($bg,array("name"=>$Wg)+$K),"DROP $bg ".idf_escape($Wg),substr(ME,0,-1),'Procedura zostaÅ‚a usuniÄ™ta.','Procedura zostaÅ‚a zmieniona.','Procedura zostaÅ‚a utworzona.',$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmieÅ„ funkcjÄ™':'ZmieÅ„ procedurÄ™').": ".h($da):(isset($_GET["function"])?'UtwÃ³rz funkcjÄ™':'UtwÃ³rz procedurÄ™')),$m);if(!$_POST&&$da!=""){$K=routine($da,$bg);$K["name"]=$da;}$jb=get_vals("SHOW CHARACTER SET");sort($jb);$cg=routine_languages();echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($cg?'JÄ™zyk'.": ".html_select("language",$cg,$K["language"]):""),'<input type="submit" value="Zapisz zmiany">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$jb,$bg);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$K["returns"],$jb);}echo'</table>
...
as$X){if($X[1]!="USAGE")$Sc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Sc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ee=$B[1];}}if($_POST&&!$m){$Fe=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Fe",ME."privileges=",'UÅ¼ytkownik zostaÅ‚ usuniÄ™ty.');else{$ve=q($_POST["user"])."@".q($_POST["host"]);$jf=$_POST["pass"];if($jf!=''&&!$_POST["hashed"]){$jf=$g->result("SELECT PASSWORD(".q($jf).")");$m=!$jf;}$_b=false;if(!$m){if($Fe!=$ve){$_b=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ve IDENTIFIED BY PASSWORD ".q($jf));$m=!$_b;}elseif($jf!=$Ee)queries("SET PASSWORD FOR $ve = ".q($jf));}if(!$m){$Yf=array();foreach($te
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'Uprawnienia uÅ¼ytkownikÃ³w'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Sc
as$_e=>$Rc){echo'<th>'.($_e!="*.*"?"<input name='objects[$r]' value='".h($_e)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$ub=>$Lb){foreach((array)$Af[$ub]as$_f=>$nb){echo"<tr".odd()."><td".($Lb?">$Lb<td":" colspan='2'").' lang="en" title="'.h($nb).'">'.h($_f);$r=0;foreach($Sc
as$_e=>$Rc){$C="'grants[$r][".h(strtoupper($_f))."]'";$Y=$Rc[strtoupper($_f)];if($ub=="Server Admin"&&$_e!=(isset($Sc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Uprawnienia'."<option value='0'".($Y=="0"?" selected":"").">".'UsuÅ„ uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$m){$Hd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Hd++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wÄ…tek.','Przerwano %d wÄ…tki.','Przerwano %d wÄ…tkÃ³w.'),$Hd),$Hd||!$_POST["kill"]);}page_header('Lista procesÃ³w',$m);echo'
<form action="" method="post">
...
';}$Kc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($Kc['sql']);break;}}if($Kc){print_fieldset("export",'Eksport'." <span id='selected2'></span>");$af=$b->dumpOutput();echo($af?html_select("output",$af,$wa["output"])." ":""),html_select("format",$Kc,$wa["format"])," <input type='submit' name='export' value='".'Eksport'."'>\n","</div></fieldset>\n";}echo(!$Tc&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",'Import',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$wa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($gc,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Bg=isset($_GET["status"]);page_header($Bg?'Status':'Zmienne');$Mh=($Bg?show_status():show_variables());if(!$Mh)echo"<p class='message'>".'Brak rekordÃ³w.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Mh
...
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$X){json_row("tables-$k",$X);json_row("size-$k",db_size($k));}json_row("");}exit;}else{$Sg=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Sg&&!$m&&!$_POST["search"]){$I=true;$ge="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ge='Tabele zostaÅ‚y oprÃ³Å¼nione.';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabele zostaÅ‚y przeniesione.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabele zostaÅ‚y skopiowane.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ge='Tabele zostaÅ‚y usuniÄ™te.';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ge='Tabele zostaÅ‚y zoptymalizowane.';}elseif(!$_POST["tables"])$ge='Brak tabel.';elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ge.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ge,$I);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$m,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabele i perspektywy'."</h3>\n";$Rg=tables_list();if(!$Rg)echo"<p class='message'>".'Brak tabel.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Wyszukaj we wszystkich tabelach'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabela','<td>'.'SkÅ‚adowanie','<td>'.'PorÃ³wnywanie znakÃ³w','<td>'.'Rozmiar danych','<td>'.'Rozmiar indeksÃ³w','<td>'.'Wolne miejsce','<td>'.'Auto Increment','<td>'.'Liczba rekordÃ³w',(support("comment")?'<td>'.'Komentarz':''),"</thead>\n";$S=0;foreach($Rg
as$C=>$U){$Ph=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($Ph?"views[]":"tables[]"),$C,in_array($C,$Sg,true),"","formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktura tabeli'.'">'.h($C).'</a>':h($C));if($Ph){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'ZmieÅ„ perspektywÄ™'.'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'PokaÅ¼ dane'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'ZmieÅ„ tabelÄ™'),"Index_length"=>array("indexes",'ZmieÅ„ indeksy'),"Data_free"=>array("edit",'Nowy rekord'),"Auto_increment"=>array("auto_increment=1&create",'ZmieÅ„ tabelÄ™'),"Rows"=>array("select",'PokaÅ¼ dane'),)as$x=>$_){$s=" id='$x-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$s title='$_[1]'>?</a>":"<span$s>?</span>"):"<td id='$x-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($Rg)),"<td>".nbsp($w=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Jh="<input type='submit' value='".'WyczyÅ›Ä‡'."'".on_help("'VACUUM'")."> ";$Me="<input type='submit' name='optimize' value='".'Optymalizuj'."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".'Zaznaczone'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Jh:($w=="pgsql"?$Jh.$Me:($w=="sql"?"<input type='submit' value='".'Analizuj'."'".on_help("'ANALYZE TABLE'")."> ".$Me."<input type='submit' name='check' value='".'SprawdÅº'."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".'Napraw'."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".'OprÃ³Å¼nij'."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().on_help("'DROP TABLE'").">\n";$j=(support("scheme")?$b->schemas():$b->databases());if(count($j)!=1&&$w!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'PrzenieÅ› do innej bazy danych'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'PrzenieÅ›'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.'UtwÃ³rz tabelÄ™'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'UtwÃ³rz perspektywÄ™'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'Procedury i funkcje'."</h3>\n";$dg=routines();if($dg){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($dg
